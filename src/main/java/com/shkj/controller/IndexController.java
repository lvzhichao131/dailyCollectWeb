...
public class IndexController {
	...
	public String VerifyData(String Data,String Token){			
		try {
			...
			String plainTextString = new String(s ,"GB2312");
			//传入的值为|分割的字符串，解析字符串
			String[] textArray = plainTextString.split("\\|");
			Map<String,String> textMap = new HashMap<String, String>();
			//解析后的字符串放入到map中
			for (String arrStr : textArray) {
				String[] val = arrStr.split("=");
				textMap.put(val[0],val[1]);
			}
			//将json字符串转化为JSONObject 
//			JSONObject jsStr = JSONObject.parseObject(textMap);
			//通过getString("")分别取出里面的信息(data);			 
			 ...
			 if(result == 0){ 
				 //取用户ID，验证是否在白名单
	       	     String ystID = textMap.get("ystID"); 
